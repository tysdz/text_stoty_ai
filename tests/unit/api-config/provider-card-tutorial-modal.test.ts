...
describe('ProviderCardShell tutorial modal', () => {
  ...
  it('mounts the tutorial modal through a portal to document.body', () => {
    ...
    const t = (key: string): string => {
      if (key === 'tutorial.button') return 'localized text'
      if (key === 'tutorial.title') return 'localized text'
      if (key === 'tutorial.subtitle') return 'localized text'
      if (key === 'tutorial.steps.ark_step1') return 'localized text API Key'
      if (key === 'tutorial.openLink') return 'localized text'
      if (key === 'tutorial.close') return 'close'
      return key
    }
    ...
    expect(html).toContain('localized text API Key')
