...
describe('async poll externalId contract', () => {
  ...
  it('rejects legacy non-standard externalId formats', () => {
    expect(() => parseExternalId('FAL:fal-ai/wan/v2.6/image-to-video:req_123')).toThrow(/localized text FAL externalId/)
    expect(() => parseExternalId('batches/legacy')).toThrow(/localized text externalId localized text/)
  })
