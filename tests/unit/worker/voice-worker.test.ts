...
describe('worker voice processor behavior', () => {
  ...
  it('VOICE_LINE: lineId/episodeId fails explicitly when missing', async () => {
  ...
  it('VOICE_LINE: passes core parameters during normal generation generateVoiceLine', async () => {
    const processor = workerState.processor
    expect(processor).toBeTruthy()
  ...
  it('VOICE_DESIGN / ASSET_HUB_VOICE_DESIGN: localized text voice design handler', async () => {
    const processor = workerState.processor
    expect(processor).toBeTruthy()
