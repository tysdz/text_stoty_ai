...
describe('script-to-storyboard atomic retry', () => {
  ...
  it('localized text clip+phase stepKey', () => {
  ...
  it('phase3 localized text phase3 localized text phase1/phase2 artifact localized text', async () => {
  ...
  it('phase2 localized text phase3 artifact localized text', async () => {
    listArtifactsMock.mockImplementation(async (params: {
      runId: string
      artifactType?: string
      refId?: string
    ...
    const runStep = vi.fn(async (_meta, _prompt, action: string) => {
      ...
      return {
        text: JSON.stringify([{ panel_number: 1, composition: 'localized text' }]),
        reasoning: '',
      }
    })
