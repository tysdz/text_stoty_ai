...
describe('worker analyze-novel behavior', () => {
  beforeEach(() => {
    ...
    prismaMock.novelPromotionProject.findUnique.mockResolvedValue({
      ...
      artStyle: 'cinematic',
      globalAssetText: 'localized text',
      characters: [{ id: 'char-existing', name: 'localized text' }],
      locations: [{ id: 'loc-existing', name: 'localized text', summary: 'old' }],
    })
    ...
    llmMock.getCompletionContent
      .mockReturnValueOnce(JSON.stringify({
        characters: [
          {
            name: 'localized text',
            aliases: ['localized textA'],
            role_level: 'main',
            personality_tags: ['calm'],
            visual_keywords: ['black hair'],
          },
        ],
      }))
      .mockReturnValueOnce(JSON.stringify({
        locations: [
          {
            ...
            descriptions: ['rainy night street [SUFFIX]'],
          },
        ],
      }))
      .mockReturnValueOnce(JSON.stringify({
  ...
  it('no global text and no episode text -> explicit error', async () => {
    ...
    await expect(handleAnalyzeNovelTask(buildJob())).rejects.toThrow('Please fill in global asset settings or script content first')
  })
    ...
    expect(prismaMock.novelPromotionCharacter.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          name: 'localized text',
          aliases: JSON.stringify(['localized textA']),
        }),
      }),
    )
