...
describe('worker script-to-storyboard behavior', () => {
  ...
  it('missing episodeId -> localized text', async () => {
    const job = buildJob({}, null)
    await expect(handleScriptToStoryboardTask(job)).rejects.toThrow('episodeId is required')
  })

  it('success path: write voice line localized text matchedPanel mapped panelId', async () => {
  ...
  it('voice retries once after parse failure and then succeeds', async () => {
    ...
    expect(reportTaskProgressMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        stepAttempt: 2,
        message: 'voice line analysis failed，preparing retry (2/2)',
      }),
    )
  })

  it('empty voice line array -> completes and clears old voice lines', async () => {
  ...
  it('phase phase retry: runs only atomic phase，does not rerun the full graph', async () => {
