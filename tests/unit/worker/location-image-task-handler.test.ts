...
describe('worker location-image-task-handler behavior', () => {
  beforeEach(() => {
    ...
    prismaMock.locationImage.findUnique.mockResolvedValue({
      ...
      imageIndex: 0,
      description: 'rainy night street',
  ...
  it('success path -> generates and persists concrete location image url', async () => {
    ...
    const animeStylePrompt = getArtStylePrompt('japanese-anime', 'vi')
    ...
    expect(sharedMock.generateLabeledImageToCos).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: `rainy night street，${animeStylePrompt}`,
        label: 'Old Town',
  ...
  it('payload artStyle overrides project artStyle in prompt', async () => {
    ...
    expect(sharedMock.generateLabeledImageToCos).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: `rainy night street，${getArtStylePrompt('realistic', 'vi')}`,
      }),
    )
  })
  ...
  it('honors requested count when location already has more slots', async () => {
    ...
    prismaMock.novelPromotionLocation.findUnique.mockResolvedValueOnce({
      ...
      name: 'Old Town',
      images: [
        { id: 'location-image-1', locationId: 'location-1', imageIndex: 0, description: 'rainy night street A' },
        { id: 'location-image-2', locationId: 'location-1', imageIndex: 1, description: 'rainy night street B' },
        { id: 'location-image-3', locationId: 'location-1', imageIndex: 2, description: 'rainy night street C' },
      ],
    })
