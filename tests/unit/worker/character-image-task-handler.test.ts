...
describe('worker character-image-task-handler behavior', () => {
  beforeEach(() => {
    ...
    prismaMock.characterAppearance.findUnique.mockResolvedValue({
      ...
      appearanceIndex: 1,
      descriptions: JSON.stringify(['localized textA']),
      description: 'localized textA',
  ...
  it('success path -> uses primary appearance as reference and persists imageUrls', async () => {
    ...
    const realisticStylePrompt = getArtStylePrompt('realistic', 'vi')
  ...
  it('payload artStyle overrides project artStyle in prompt', async () => {
    ...
    expect(generationInput.prompt).toContain(getArtStylePrompt('japanese-anime', 'vi'))
    expect(generationInput.prompt).not.toContain(getArtStylePrompt('realistic', 'vi'))
  })
