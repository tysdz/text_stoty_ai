...
describe('voice-design-shared', () => {
  ...
  it('generates the requested number of voice options with default preview text fallback', async () => {
    ...
    const result = await generateVoiceDesignOptions({
      ...
      previewText: '   ',
      defaultPreviewText: 'default preview text',
      onDesignVoice,
    ...
    expect(onDesignVoice.mock.calls).toEqual([
      [{ voicePrompt: 'warm female voice', previewText: 'default preview text', preferredName: 'preferred-1', language: 'vi' }],
      [{ voicePrompt: 'warm female voice', previewText: 'default preview text', preferredName: 'preferred-2', language: 'vi' }],
      [{ voicePrompt: 'warm female voice', previewText: 'default preview text', preferredName: 'preferred-3', language: 'vi' }],
      [{ voicePrompt: 'warm female voice', previewText: 'default preview text', preferredName: 'preferred-4', language: 'vi' }],
    ])
  })
