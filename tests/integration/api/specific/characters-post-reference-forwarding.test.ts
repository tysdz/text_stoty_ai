  ...
  it('forwards locale and accept-language into background reference task payload', async () => {
    ...
    const req = buildMockRequest({
      ...
      body: {
        ...
        customDescription: 'calm，black hair',
        count: 5,
      },
    })
    ...
    expect((calledInit?.headers as Record<string, string>)['Accept-Language']).toBe('vi-VN,vi;q=0.9')
    ...
    expect(forwarded.locale).toBe('vi')
    expect(forwarded.meta?.locale).toBe('vi')
    expect(forwarded.customDescription).toBe('calm，black hair')
