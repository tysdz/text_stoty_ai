...
const languageDirectivePattern = /use Vietnamese|output in Vietnamese|Vietnamese output/i
...
function collectLanguageDirectiveViolations() {
  ...
  for (const filePath of promptFiles) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      const line = lines[index]
      if (languageDirectivePattern.test(line)) {
        violations.push(`${relPath}:${index + 1} English template cannot require Vietnamese output`)
      }
    }
  }
...
function collectLegacyPromptFiles() {
  return walk(path.join(root, 'lib', 'prompts'))
    ...
    .filter((relPath) => !relPath.startsWith('lib/prompts/skills/'))
    .filter((relPath) => relPath.endsWith('.txt') && !relPath.endsWith('.vi.txt') && !relPath.endsWith('.en.txt'))
}
...
function verifyPromptCatalogCoverage() {
  ...
  for (const stem of stems) {
    const zhPath = path.join(root, 'lib', 'prompts', `${stem}.vi.txt`)
    const enPath = path.join(root, 'lib', 'prompts', `${stem}.en.txt`)
    if (!fs.existsSync(zhPath)) {
      missing.push(`missing vi template: lib/prompts/${stem}.vi.txt`)
    }
