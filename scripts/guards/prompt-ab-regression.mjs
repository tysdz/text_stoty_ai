...
for (const entry of entries) {
  const zhPath = path.join(root, 'lib', 'prompts', `${entry.pathStem}.vi.txt`)
  const enPath = path.join(root, 'lib', 'prompts', `${entry.pathStem}.en.txt`)
  if (!fs.existsSync(zhPath)) {
    violations.push(`missing vi template: lib/prompts/${entry.pathStem}.vi.txt`)
    continue
  }
  ...
  for (const key of missingInZh) {
    violations.push(`missing {${key}} in vi template: lib/prompts/${entry.pathStem}.vi.txt`)
  }
  for (const key of missingInEn) {
    violations.push(`missing {${key}} in en template: lib/prompts/${entry.pathStem}.en.txt`)
  }
  for (const key of extraInZh) {
    violations.push(`unexpected {${key}} in vi template: lib/prompts/${entry.pathStem}.vi.txt`)
  }
  ...
  for (const key of zhOnly) {
    violations.push(`placeholder {${key}} exists only in vi template: ${entry.pathStem}`)
  }
  ...
  if (unresolvedZh.length > 0) {
    violations.push(`unresolved placeholders in vi template: ${entry.pathStem} -> ${unresolvedZh.join(', ')}`)
  }
  ...
  for (const [key, sample] of Object.entries(variables)) {
    if (!renderedZh.includes(sample)) {
      violations.push(`vi template variable not used after render: ${entry.pathStem}.{${key}}`)
    }
