// Next.js Instrumentation - localized text
// https://nextjs.org/docs/app/building-your-application/optimizing/instrumentation

export async function register() {
  // localized text Edge Runtime localized text，localized text Prisma（localized text）
  if (process.env.NEXT_RUNTIME === 'edge') {
    return
  }

  // localized text Node.js localized text
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    const { prisma } = await import('@/lib/prisma')
    const { logInfo: _ulogInfo, logError: _ulogError } = await import('@/lib/logging/core')

    // Phase 1: localized text processing localized text queued
    try {
      const resetResult = await prisma.task.updateMany({
        ...
        data: {
          ...
          startedAt: null,
          heartbeatAt: null,
          // localized text externalId，localized text worker localized text
          // localized text API（Kling localized text），localized text
        },
      })
    ...
    // Phase 2: localized text queued localized text BullMQ localized text
    // localized text Redis localized text DB localized text queued localized text BullMQ Job localized text
    try {
    ...
    // ─── Phase 3: localized text Task Watchdog（DB ↔ BullMQ localized text）───
    try {
