...
const SWITCH_CONFIRM_COPY: Record<Locale, { title: string; message: string; action: string; cancel: string; triggerLabel: string }> = {
    vi: {
        title: 'Đổi ngôn ngữ?',
        message:
            'Chuyển sang {targetLanguage} sẽ cập nhật giao diện, prompt, nội dung kịch bản và kết quả workflow. Tiếp tục?',
        action: 'Đổi ngay',
        cancel: 'Hủy',
...
export default function LanguageSwitcher() {
    ...
    if (!isSupportedLocale(locale)) {
        throw new Error('LanguageSwitcher requires locale to be vi or en')
    }
