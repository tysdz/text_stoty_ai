...
async function handleInsertPanelTask(job: Job<TaskJobData>) {
  ...
  const charactersFullDescription = (projectData.characters || [])
    ...
    .map((character) => {
      ...
      if (appearances.length === 0) return `${character.name}: localized text`
      const appearanceText = appearances
        .map((appearance) => {
          ...
          const selectedDescription = descriptions[selectedIndex] || appearance.description || 'localized text'
  ...
  const locationsDescription = projectLocations
    ...
    .map((location) => {
      ...
      return `${location.name}: ${selectedImage?.description || 'localized text'}`
    })
    .join('\n') || 'none'
  const propsDescription = projectProps
    .filter((prop) => relatedProps.length === 0 || relatedProps.includes(prop.name))
    .map((prop) => `${prop.name}: ${prop.summary || 'localized text'}`)
    .join('\n') || 'none'
