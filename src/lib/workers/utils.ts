...
/**
 * localized text DB localized text externalId（localized text，localized text API）
 */
...
export async function resolveImageSourceFromGeneration(
  ...
  // localized text：localized text DB localized text externalId，localized text，localized text API
...
/**
 * localized text：localized text URL localized text。
 *
 * - localized text（result.imageUrls）→ localized text
 * - localized text（result.imageUrl / result.imageBase64）→ localized text [url] localized text
 * - localized text：localized text URL，localized text [url]
 *
 * localized text resolveImageSourceFromGeneration（localized text），
 * localized text。
 */
export async function resolveImageSourcesFromGeneration(
  ...
  // localized text：localized text DB localized text externalId，localized text（localized text）
  if (allowTaskExternalIdResume) {
    const resumeExternalId = await getTaskExistingExternalId(job.data.taskId)
    if (resumeExternalId) {
      logger.info({
...
export async function resolveVideoSourceFromGeneration(
  ...
  // localized text：localized text DB localized text externalId，localized text，localized text API（localized text）
  const resumeExternalId = await getTaskExistingExternalId(job.data.taskId)
  if (resumeExternalId) {
    logger.info({
...
export async function resolveLipSyncVideoSource(
  ...
  // localized text：localized text DB localized text externalId，localized text，localized text（localized text）
  const resumeExternalId = await getTaskExistingExternalId(job.data.taskId)
  if (resumeExternalId) {
    logger.info({
...
/**
 * localized text，localized text base64 data URL
 * localized text，localized text AI localized text
 */
