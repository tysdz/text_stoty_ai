...
export async function handleAnalyzeGlobalTask(job: Job<TaskJobData>) {
  ...
  if (!allContent.trim()) {
    throw new Error('no content to analyze，localized text')
  }
  ...
  await reportTaskProgress(job, 10, {
    ...
    displayMode: 'detail',
    message: `localized text ${chunks.length} localized text`,
  })
  ...
  try {
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      await reportTaskProgress(job, progress, {
        ...
        stageLabel: 'localized text',
        displayMode: 'detail',
        message: `localized text ${i + 1}/${chunks.length}`,
        stepId: `analyze_global_chunk_${i + 1}`,
        stepTitle: `localized text ${i + 1}/${chunks.length}`,
        stepIndex: i + 1,
