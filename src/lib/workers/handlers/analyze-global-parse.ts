...
const INVALID_LOCATION_KEYWORDS = ['localized text', 'localized text', 'localized text', 'localized text', 'localized text', 'localized text']
...
export function buildCharactersLibInfo(characters: CharacterBrief[]): string {
  if (characters.length === 0) return 'localized text'
  return characters
    .map((c, i) => {
      const aliasStr = c.aliases.length > 0 ? `localized text：${c.aliases.join('、')}` : 'localized text：none'
      const introStr = c.introduction ? `localized text：${c.introduction}` : 'localized text：localized text'
      return `${i + 1}. ${c.name}\n   ${aliasStr}\n   ${introStr}`
    })
    .join('\n\n')
}
