...
export async function handleAnalyzeShotVariantsTask(job: Job<TaskJobData>, payload: AnyObj) {
  ...
  if (!panel.imageUrl) throw new Error('localized text，localized text')
  ...
  const prompt = buildPrompt({
    ...
    variables: {
      ...
      shot_type: panel.shotType || 'localized text',
      camera_move: panel.cameraMove || 'localized text',
      location: panel.location || 'localized text',
  ...
  if (!Array.isArray(suggestions) || suggestions.length < 3) {
    throw new Error('not enough generated variants')
  }
