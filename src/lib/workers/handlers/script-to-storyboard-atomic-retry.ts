...
export async function runScriptToStoryboardAtomicRetry(params: {
  ...
  if (params.retryTarget.phase === 'phase1') {
    ...
    const charactersLibName = (params.novelPromotionData.characters || []).map((item) => item.name).join(', ') || 'none'
    const locationsLibName = (params.novelPromotionData.locations || []).map((item) => item.name).join(', ') || 'none'
    ...
    if (screenplay) {
      phase1Prompt = phase1Prompt.replace('{clip_content}', `【localized text】\n${JSON.stringify(screenplay, null, 2)}`)
    } else {
