...
function buildPanelPromptContext(params: {
  ...
  const characterContexts = panelCharacters.map((reference) => {
    ...
    return {
      ...
      description: matchedAppearance ? pickAppearanceDescription(matchedAppearance) : 'localized text',
    }
  })
...
export async function handlePanelImageTask(job: Job<TaskJobData>) {
  ...
  const prompt = buildPanelPrompt({
    ...
    aspectRatio,
    styleText: artStyle || 'localized text',
  ...
  for (let i = 0; i < candidateCount; i++) {
    ...
    const source = await resolveImageSourceFromGeneration(job, {
      ...
      // localized text，localized text task.externalId localized text。
