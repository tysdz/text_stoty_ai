...
export async function handleModifyAssetImageTask(job: Job<TaskJobData>) {
  ...
  // localized text payload.generationOptions localized text resolution（localized text route localized text buildImageBillingPayload localized text）
  // localized text getModelResolution localized text，localized text capabilityDefaults/capabilityOverrides localized text
  ...
  if (type === 'character') {
    ...
    const prompt = `localized text，localized text：\n${modifyInstruction}`
    ...
    const label = `${appearance.character?.name || 'Character'} - ${appearance.changeReason || 'localized text'}`
    ...
    if (currentDescription && modifyInstruction) {
      try {
        ...
        logger.warn({ message: 'localized text，localized text', details: { error: String(err) } })
      }
    }
