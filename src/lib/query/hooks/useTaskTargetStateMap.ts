...
/** localized text targets localized text（targets localized text <= TARGET_STATE_CHUNK_SIZE） */
...
async function flushTaskTargetStateBatch(projectId: string) {
  ...
  try {
    // localized text targets localized text TARGET_STATE_CHUNK_SIZE localized text，localized text
    ...
    // localized text
    // localized text targetQueryKey（localized text types）localized text，localized text (targetType, targetId)
    // localized text types localized text（localized text image localized text processing localized text lip_sync localized text idle localized text）
