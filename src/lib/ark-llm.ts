/**
 * localized text Ark LLM (Responses API) localized text
 */
...
export async function arkResponsesCompletion(options: ArkResponsesOptions): Promise<ArkResponsesResult> {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Ark Responses localized text: ${response.status} - ${errorText}`)
    }
...
// ============================================================
// localized text：OpenAI messages → Responses API input
// ============================================================
...
/**
 * localized text OpenAI localized text messages localized text Responses API localized text input localized text。
 * system localized text user localized text（Responses API localized text instructions localized text，
 * localized text user localized text）。
 */
export function convertChatMessagesToArkInput(messages: ChatMessage[]): ArkResponsesInputItem[] {
    ...
    for (const msg of messages) {
        ...
        // localized text system localized text user localized text
...
export function buildArkThinkingParam(
    ...
    // Ark Responses localized text reasoning_effort localized text。
    // localized text 400，localized text thinking.type。
    return { thinking: { type: reasoning ? 'enabled' : 'disabled' } }
}

// ============================================================
// localized text Responses API
// ============================================================
...
/**
 * localized text Responses API，back AsyncIterable<ArkStreamDelta> + localized text Promise。
 */
export function arkResponsesStream(options: ArkResponsesOptions & { temperature?: number }): {
    ...
    async function* generateStream(): AsyncIterable<ArkStreamDelta> {
        ...
        if (!response.ok) {
            const errorText = await response.text()
            const err = new Error(`Ark Responses localized text: ${response.status} - ${errorText}`)
            rejectResult(err)
            throw err
        }
