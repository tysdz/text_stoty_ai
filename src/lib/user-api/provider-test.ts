...
async function testCompatibleProvider(baseUrl: string, apiKey: string, llmModel?: string): Promise<TestProviderResult> {
  ...
  if (!fallbackModel) {
    steps.push({
      name: 'textGen',
      status: 'fail',
      message: 'No free probe endpoint detected. Please configure an LLM model first, then retry / localized text，localized text LLM localized text',
    })
    return { success: false, steps }
  }
...
async function testArkProvider(apiKey: string): Promise<TestProviderResult> {
  ...
  // localized text src/lib/ark-llm.ts localized text arkResponsesCompletion localized text，localized text Responses API
  ...
  try {
    ...
    // localized text ark-llm.ts localized text output_text
...
async function testFalProvider(apiKey: string): Promise<TestProviderResult> {
  ...
  // 🔥 localized text GET /v1/models localized text API Key，localized text
  try {
...
async function testViduProvider(apiKey: string): Promise<TestProviderResult> {
  ...
  // 🔥 localized text GET /ent/v2/credits localized text，localized text
  try {
...
function toErrorMessage(error: unknown): string {
  if (error instanceof Error) {
    ...
    if (error.message.includes('fetch failed') || error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND'))
      return 'Network error — check your internet connection / localized text，localized text'
    if (error.message.includes('Connection error')) return 'Network error — temporary connection failure, please retry / localized text，localized text'
