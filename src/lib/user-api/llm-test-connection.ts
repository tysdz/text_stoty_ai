...
function normalizeProvider(payload: TestConnectionPayload): SupportedProvider {
  ...
  if (!provider) {
    ...
    throw new ApiError('INVALID_PARAMS', { message: 'localized text provider' })
  }
...
function requireApiKey(payload: TestConnectionPayload): string {
  ...
  if (!apiKey) {
    throw new ApiError('INVALID_PARAMS', { message: 'localized text apiKey' })
  }
  return apiKey
}
...
function requireBaseUrl(payload: TestConnectionPayload): string {
  ...
  if (!baseUrl) {
    throw new ApiError('INVALID_PARAMS', { message: 'localized text baseUrl' })
  }
  return baseUrl
}
...
async function testGoogleAI(apiKey: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Google AI localized text: ${error}`)
  }
}
...
async function testOpenAICompatibleConnection(params: {
  ...
  if (params.model) {
    const response = await client.chat.completions.create({
      model: params.model,
      messages: [{ role: 'user', content: '1+1localized text？localized text' }],
      max_tokens: 10,
      temperature: 0,
    })
...
export async function testLlmConnection(payload: TestConnectionPayload): Promise<LlmConnectionTestResult> {
  ...
  switch (provider) {
    case 'openrouter': {
      ...
      return { provider, message: 'openrouter localized text', ...tested }
    }
    case 'google':
      await testGoogleAI(apiKey)
      return { provider, message: 'google localized text' }
    case 'anthropic': {
      const tested = await testOpenAICompatibleConnection({
        apiKey,
      ...
      return { provider, message: 'anthropic localized text', ...tested }
    }
    case 'openai': {
      const tested = await testOpenAICompatibleConnection({
        apiKey,
      ...
      return { provider, message: 'openai localized text', ...tested }
    }
    case 'bailian': {
      const tested = await testBailianProbe(apiKey)
      return { provider, message: 'bailian localized text', ...tested }
    }
    case 'siliconflow': {
      const tested = await testSiliconFlowProbe(apiKey)
      return { provider, message: 'siliconflow localized text', ...tested }
    }
    case 'openai-compatible': {
      const tested = await testOpenAICompatibleConnection({
        apiKey,
      ...
      return { provider, message: 'openai-compatible localized text', ...tested }
    }
    case 'gemini-compatible': {
      const tested = await testOpenAICompatibleConnection({
        apiKey,
      ...
      return { provider, message: 'gemini-compatible localized text', ...tested }
    }
    case 'custom': {
      const tested = await testOpenAICompatibleConnection({
        apiKey,
      ...
      return { provider, message: 'custom localized text', ...tested }
    }
  }
}
