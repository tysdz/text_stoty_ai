...
/**
 * localized text，localized text JSON localized text billingMeta
 * localized text unit localized text：
 *   image  → "3localized text · 2K"
 *   video  → "5localized text · 720p"
 *   token  → "1500 tokens"
 *   second → "30localized text"
 *   call   → "1localized text"
 */
export function buildBillingMeta(params: {
  ...
  // localized text model composite ID localized text "provider:xxx::model" → "model"
  ...
  // localized text pricingSelections localized text capability localized text（localized text、localized text/localized text）
...
export async function recordUsageCostOnly(
  ...
  if (hasProject) {
    ...
    _ulogInfo(`[localized text] localized text UsageCost localized text (projectId=${params.projectId})，localized text`)
  }
  ...
  _ulogInfo(`[localized text] ${params.action} - ${params.model} - ¥${params.cost.toFixed(4)} (localized text${hasProject ? '' : '，localized text'})`)
}
