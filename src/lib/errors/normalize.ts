...
/**
 * MODEL_NOT_CONFIGURED: localized text
 * localized text：model_not_found / model_not_configured / no xxx model is enabled
 */
...
function inferCodeFromMessage(message: string): UnifiedErrorCode | null {
  ...
  if (containsAny(message, ['task cancelled', 'canceled by user', 'cancelled by user', 'localized text'])) return 'CONFLICT'
  if (containsAny(message, ['unauthorized', 'not authenticated', 'need login', '401'])) return 'UNAUTHORIZED'
  // AccountOverdueError（ARK localized text 403）localized text FORBIDDEN localized text
  ...
  if (containsAny(message, ['not found', 'localized text', 'missing record'])) return 'NOT_FOUND'
  ...
  if (containsAny(message, ['insufficient balance', 'creditinsufficient', 'balance is not enough', '402', 'insufficient credits', 'insufficient balance', 'localized text', 'localized text'])) return 'INSUFFICIENT_BALANCE'
  if (containsAny(message, ['sensitive', 'unsafe', 'safety', 'blocked', 'prohibited', 'policy_violation', 'moderation', 'harm', 'localized text', 'localized text', 'localized text', 'localized text', 'localized text']) && !containsAny(message, ['case-sensitive', 'case sensitive'])) return 'SENSITIVE_CONTENT'
...
export function normalizeAnyError(input: unknown, options: NormalizeOptions = {}): NormalizedError {
  ...
  if (typeof errorLike.status === 'number') {
    ...
    // 403 localized text（AccountOverdueError），localized text
