...
function isRecoverableJsonParseError(error: unknown, normalizedMessage: string): boolean {
  if (normalizedMessage.includes('ark responses localized text')) return false
...
export async function runStoryToScriptOrchestrator(
  ...
  const basePropsText = baseProps.length > 0 ? baseProps.join('、') : 'none'
  ...
  onLog?.('localized text1：Character/Location/prop analysis（localized text）')
  ...
  onLog?.('localized text2：localized text（localized text1localized text）', {
    charactersLibName,
    locationsLibName,
  })
  ...
  onLog?.('localized text3：localized text（localized text）', { clipCount: clipList.length })
