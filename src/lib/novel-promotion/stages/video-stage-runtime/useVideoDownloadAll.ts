...
export function useVideoDownloadAll({
  ...
  const handleDownloadAllVideos = useCallback(async () => {
    ...
    try {
      ...
      _ulogInfo('[localized text] localized textURLlocalized text...')
      ...
      _ulogInfo(`[localized text] localized text ${videos.length} localized text，localized text...`)
      setDownloadProgress({ current: 0, total: videos.length })

      const zip = new JSZip()
      for (let index = 0; index < videos.length; index += 1) {
        const video = videos[index]
        _ulogInfo(`[localized text] download ${index + 1}/${videos.length}: ${video.fileName}`)
        ...
        try {
          ...
          _ulogError(`[localized text] localized text: ${video.fileName}`, error)
        }
      }

      _ulogInfo('[localized text] generate ZIP localized text...')
      ...
      _ulogError('[localized text] error:', error)
