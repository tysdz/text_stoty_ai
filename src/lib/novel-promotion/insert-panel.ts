const DEFAULT_INSERT_PANEL_USER_INPUT = {
  vi: 'Automatically analyze surrounding shots and insert a naturally connected new panel。',
...
function isViLocale(locale: string | undefined): boolean {
...
export function resolveInsertPanelUserInput(payload: Record<string, unknown>, locale?: string): string {
  ...
  return isViLocale(locale)
    ? DEFAULT_INSERT_PANEL_USER_INPUT.vi
