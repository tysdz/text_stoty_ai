...
export async function chatCompletion(
  ...
  if (!model) {
    _ulogError('[LLM] localized text，localized text:', new Error().stack)
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    ...
    try {
      ...
      if (!isOpenRouter) {
        ...
        // localized text OpenAI localized text forceReasoning/reasoningEffort
        // gemini-compatible localized text OAI-compat localized text
      ...
      if (errorBody?.message === 'PROHIBITED_CONTENT' || errorBody?.code === 502) {
        _ulogError('[LLM] ❌ localized text - Google AI Studio localized text')
        throw new Error('SENSITIVE_CONTENT: localized text,localized text。localized text')
      }

      // Google Gemini localized text，localized text（localized text，localized text）
      if (error instanceof GoogleEmptyResponseError) {
        _ulogWarn(`[LLM] Google localized text，localized text (${attempt}/${maxRetries + 1}): ${errorMessage(error)}`)
      ...
      _ulogWarn(`[LLM] localized text (${attempt}/${maxRetries + 1}): ${errorMessage(error)}`)
  ...
  throw lastError || new Error('LLM localized text')
}
