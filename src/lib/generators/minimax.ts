...
/**
 * MiniMax (localized text) localized text
 * 
 * localized text：
 * localized text：MiniMax-Hailuo-2.3, MiniMax-Hailuo-2.3-Fast, MiniMax-Hailuo-02, T2V-01, T2V-01-Director
 */
...
export class MinimaxVideoGenerator extends BaseVideoGenerator {
    protected async doGenerate(params: VideoGenerateParams): Promise<GenerateResult> {
        ...
        // aspectRatio localized text worker localized text（localized text videoRatio），
        // MiniMax localized text（localized text resolution localized text），localized text。
        const allowedOptionKeys = new Set([
            ...
            'generateAudio',
            'lastFrameImageUrl',
            'aspectRatio',  // localized text API，localized text worker localized text
        ])
        ...
        if (modelSpec.supportsImageInput && hasFirstFrameImage) {
            ...
            if (generationMode === 'firstlastframe' && isNonEmptyString(lastFrameImageUrl)) {
                ...
                _ulogInfo(`${logPrefix} localized text (localized textData URL)`)
            } else {
                _ulogInfo(`${logPrefix} localized text (localized textData URL)`)
            }
        }
        ...
        _ulogInfo(
            `${logPrefix} localized text，mode=${generationMode}，duration=${duration ?? '(provider default)'}s，resolution=${normalizedResolution ?? '(provider default)'}`,
        )
        ...
        try {
            ...
            if (!response.ok) {
                ...
                _ulogError(`${logPrefix} APIlocalized text:`, response.status, errorText)
            ...
            if (data.base_resp?.status_code !== 0) {
                ...
                _ulogError(`${logPrefix} localized text:`, errMsg)
                throw new Error(`MiniMax: ${errMsg}`)
            }

            const taskId = data.task_id
            if (!taskId) {
                _ulogError(`${logPrefix} localized text task_id:`, data)
                throw new Error('MiniMaxlocalized texttask_id')
            }
