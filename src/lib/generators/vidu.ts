...
export class ViduVideoGenerator extends BaseVideoGenerator {
    protected async doGenerate(params: VideoGenerateParams): Promise<GenerateResult> {
        ...
        _ulogInfo(`${logPrefix} - localized text:`, JSON.stringify(requestBody, null, 2))
        ...
        try {
            ...
            if (!taskId) {
                ...
                throw new Error('Vidulocalized texttask_id')
            }

            const state = data.state
            if (state === 'failed') {
                _ulogError(`${logPrefix} localized text:`, data)
                throw new Error('Vidu: localized text')
            }

            _ulogInfo(`${logPrefix} localized text，task_id=${taskId}, state=${state}`)
