...
function writeProjectLogLine(line: string, projectId: string | undefined, moduleName: string | undefined): void {
  ...
  // localized text（localized text app.log）
...
function nowChinaISOString(): string {
  const now = new Date()
  // UTC+8 localized text（localized text）
  const offsetMs = 8 * 60 * 60 * 1000
  const cstTime = new Date(now.getTime() + offsetMs)
  // toISOString() back UTC localized text，localized text Z localized text +08:00 localized text
