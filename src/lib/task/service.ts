...
/**
 * localized text BullMQ Job localized text。
 * localized text（localized text Redis localized text）localized text true，localized text。
 */
async function verifyJobAlive(taskId: string): Promise<boolean> {
  try {
    ...
    // Redis localized text → localized text DB localized text
    return true
  }
}
    ...
    if (existing) {
      if (isActiveStatus(existing.status)) {
        if (!hasTaskLocale(existing.payload)) {
          ...
          // localized text BullMQ Job localized text，localized text DB localized text
          ...
          // Job localized text（terminal / missing）→ localized text，localized text dedupeKey，localized text
          await model.update({
      ...
      if (collided) {
        if (isActiveStatus(collided.status)) {
          if (!hasTaskLocale(collided.payload)) {
            ...
            // P2002 localized text：localized text BullMQ Job localized text
