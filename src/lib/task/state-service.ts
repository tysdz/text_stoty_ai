...
/**
 * localized text OR localized text。
 * localized text OR localized text + ORDER BY localized text MySQL sort buffer localized text（Error 1038）。
 */
...
export async function queryTaskTargetStates(params: {
  ...
  // localized text，localized text MySQL sort buffer localized text
  const allRows: Array<{
    id: string
    type: string
    status: string
  ...
  for (let i = 0; i < pairs.length; i += QUERY_BATCH_SIZE) {
    ...
    const rows = await prisma.task.findMany({
      ...
      // localized text，localized text sort buffer localized text
      select: {
        id: true,
        type: true,
        status: true,
  ...
  // localized text updatedAt desc localized text（localized text target localized text）
  ...
  // localized text updatedAt desc localized text
