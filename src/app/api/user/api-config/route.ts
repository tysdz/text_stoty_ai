 ...
 * PUT  - save/localized text(localized text)
 */
...
function applyVideoDurationRangeIfNeeded(input: {
  ...
  // Ark/localized text：localized text tier localized text，localized text 5 localized text，localized text [localized text, localized text]。
...
export const GET = apiHandler(async () => {
  ...
  // localized text gemini-compatible provider，localized text Google preset localized text（disabled，localized text capabilities）
  // gemini-compatible localized text baseURL localized text key，localized text Google localized text
  ...
  for (const p of providers) {
    ...
    for (const preset of GEMINI_COMPATIBLE_PRESETS) {
      ...
      const base: StoredModel = {
        ...
        provider: p.id,
        price: 0,
        // alias localized text google catalog localized text capabilities
