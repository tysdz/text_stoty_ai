...
export function CharacterCard({ character, onImageClick, onImageEdit, onVoiceDesign, onEdit, onVoiceSelect }: CharacterCardProps) {
    ...
    // localized text（localized text query localized text）
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-1">
                        <ImageGenerationInlineCountButton
                            ...
                            onClick={() => {
                                _ulogInfo('[CharacterCard] localized text - localized text, characterId:', character.id, 'appearanceCount:', appearanceCount)
                        ...
                        <button onClick={(e) => {
                            e.stopPropagation()
                            _ulogInfo('[CharacterCard] localized text - localized text, characterId:', character.id, 'appearanceCount:', appearanceCount, 'showDeleteMenu:', showDeleteMenu)
                    ...
                    <div className="flex gap-1 mt-2 overflow-x-auto">
                        {character.appearances.map((app, index) => (
                            <button key={app.id} onClick={() => setActiveAppearance(index)} className={`glass-btn-base px-2 py-0.5 text-xs rounded-full whitespace-nowrap ${index === activeAppearance ? 'glass-btn-primary' : 'glass-btn-soft text-[var(--glass-text-secondary)]'}`}>
                                {app.changeReason || `localized text ${app.appearanceIndex}`}
                            </button>
                        ))}
                    </div>
                )}
