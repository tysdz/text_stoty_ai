...
interface Project {
  ...
  createdAt: string
  updatedAt: string
  totalCost?: number  // localized text（CNY）
  stats?: ProjectStats
}
...
const PAGE_SIZE = 7 // localized text8localized text，4localized text2localized text
...
export default function WorkspacePage() {
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch('/api/projects', {
        ...
        body: JSON.stringify({
          ...formData,
          mode: 'novel-promotion' // localized text novel-promotion
        })
      })
      ...
      if (response.ok) {
        ...
        if (preferenceResponse.ok) {
          ...
          _ulogError('localized text:', { status: preferenceResponse.status })
        }
  ...
  const formatDate = (dateString: string) => {
    ...
    // localized text (UTC+8)
    const beijingTime = new Date(date.getTime() + 8 * 60 * 60 * 1000)
    return beijingTime.toLocaleDateString('vi-VN', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
  ...
  const openDeleteConfirm = (project: Project, e: React.MouseEvent) => {
    e.preventDefault()  // localized text Link localized text
    e.stopPropagation()
  ...
  return (
    <div className="glass-page min-h-screen">
            ...
            projects.map((project) => (
              ...
              >
                ...
                <div className="p-5 relative z-10">
                  ...
                  {/* localized text：localized text，fallback localized text */}
      ...
      {/* Create Project Modal - localized text，localized text */}
      {showCreateModal && (
