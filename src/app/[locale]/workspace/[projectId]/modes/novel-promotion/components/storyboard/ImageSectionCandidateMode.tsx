      ...
      <div className="absolute bottom-2 left-2 right-2 glass-surface-soft border border-[var(--glass-stroke-base)] p-2 rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <button
              onClick={async () => {
                _ulogInfo('[ImageSection] 🎯 localized text')
                _ulogInfo('[ImageSection] panelId:', panelId)
                _ulogInfo('[ImageSection] localized text:', safeSelectedIndex)
                _ulogInfo('[ImageSection] localized text URL:', validCandidates[safeSelectedIndex])
                setIsConfirming(true)
                try {
                  await onConfirmCandidate(panelId, validCandidates[safeSelectedIndex])
                  _ulogInfo('[ImageSection] ✅ localized text')
                } catch (error) {
                  _ulogError('[ImageSection] ❌ localized text:', error)
                  setIsConfirming(false)
                  _ulogInfo('[ImageSection] isConfirming localized text false (localized text)')
                }
              }}
              disabled={isConfirming}
