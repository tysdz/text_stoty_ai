...
export function useProviders(): UseProvidersReturn {
    ...
    // localized text refs，localized text
    ...
    async function fetchConfig() {
        ...
        try {
            ...
            // localized text，localized text savedProviders localized text（localized text）
            ...
            const customModels = savedModels.filter((m: CustomModel) =>
                ...
                // localized text enabled localized text（localized text disabled presets localized text enabled: false）
            ...
            if (loadedSuccessfully) {
                // localized text initialized，localized text
                setTimeout(() => {
    ...
    /**
     * localized text：localized text ref localized text，localized text（localized text）。
     * localized text：localized text「localized text」，localized text「saved」。
     */
    ...
    }, []) // localized text，localized text ref localized text
    ...
    // capabilityFieldsToDefault：localized text option write capabilityDefaults（localized text）
    ...
    const addProvider = useCallback((provider: Omit<Provider, 'hasApiKey'>) => {
        setProviders(prev => {
            ...
            if (providerKey === 'gemini-compatible') {
                // localized text refetch：localized text capabilities localized text Google localized text（disabled）
    ...
    const deleteProvider = useCallback((providerId: string) => {
        ...
        if (confirm(t('confirmDeleteProvider'))) {
            ...
            setModels(prev => {
                ...
                void performSave(undefined, true) // localized text：localized text
                return nextModels
            })
        }
    }, [t, performSave])
